  ...
  async function openEntry(id) {
    ...
    const overlay = openModal(`
      <div class="modal-header">
        <h3>History &mdash; ${escapeHtml(entry.ts)} by ${escapeHtml(entry.actor || 'unknown')}</h3>
      ...
      <div class="modal-body">
        ...
        ` : emptyState({ title: 'No files in this snapshot' })}
      </div>
    ...
    if (filenames.length) {
      ...
      overlay.querySelectorAll('#hist-yaml-tabs .yaml-tab').forEach(tab => tab.addEventListener('click', () => showFile(tab.dataset.file)));
      overlay.querySelector('#btn-download-hist-file').addEventListener('click', () => downloadTextFile(activeFile, files[activeFile] || ''));
      showFile(activeFile);
    }
  }
