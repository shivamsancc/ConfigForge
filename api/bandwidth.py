"""
FastAPI router for /api/bandwidth.

Endpoints
---------
GET    /api/bandwidth                        → {rows: [...]}
POST   /api/bandwidth         body: {row, _actor}        → {row}
DELETE /api/bandwidth/{id}    query: _actor              → {deleted: id}
POST   /api/bandwidth/import  body: {rows, mode, _actor} → {imported, mode}
POST   /api/bandwidth/validate-import  body: {rows, mode} → {findings}
"""
